...
df <- read.csv("data-analysis/btc_1d_data_2018_to_2025.csv")
df$Date <- as.Date(substr(df$Open.time, 1, 10))
df <- df[order(df$Date), ]
df <- df[!duplicated(df$Date), ]
stopifnot(all(df$Close > 0))
prices <- df$Close
...
#   p = plag, lambda = lambda_grid, c_scale = 1, val_spacing = plag + 1L,
#   verbose = FALSE, max_iter = 5000
# )
# best_lambda <- cv_fit$best$lambda
# cat(sprintf("CV selected lambda = %.4f\n", best_lambda))

cat("Fitting H-SBAR with best lambda...\n")
fit <- hsbar(x_sq, p = plag, lambda = 0.2, c_scale = 1, max_iter = 5000, verbose = FALSE)
...
# Plots
pdf("data-analysis/btc_hsbar.pdf", width = 10, height = 13)
...
plot(dates_r, r_tilde,
  ...
  xaxt = "n", xlab = "", ylab = "Log return", main = "BTC/USD daily log returns"
)
add_year_axis()
...
dev.off()
cat("Plots saved to data-analysis/btc_hsbar.pdf\n")
